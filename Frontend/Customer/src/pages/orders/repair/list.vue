...
<div class="width-container pad-05">
    <h2 class="margin-bottom-05">{{locale.repairOrder.plural}}</h2>
...
<Fetch :url="`/customer/orders/repair/${props.tab}`"
    ...
    <Pagination :items="data.items" :page="props.page" 
        ...
        <template v-slot:repeating="{ item }">
            <RepairOrderView :order="item" @click="()=> goToItem(item)" class="card-card pad-1 margin-bottom-1" />
        </template>
    </Pagination>
    </template>
</Fetch>
...
function goToPage (page) {
    if (page != null && page != undefined)
        router.push(`/orders/repair/${props.tab}?page=${page}`)
}

function goToItem (item) {
    router.push(`/orders/repair/view/${item?.id}`)
}
