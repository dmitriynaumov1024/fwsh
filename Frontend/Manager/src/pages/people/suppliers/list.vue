...
<Pagination v-if="data.items?.length"
    ...
    <template #title>
        <h2 class="margin-bottom-1">
            {{locale.supplier.plural}} &ndash; {{locale.common.page}} {{props.page}}
        </h2>
    </template>
</Pagination>
...
watch(() => props.page, getSuppliers, { immediate: true })

function goToPrevious() {
    if (data.previous != null)
        router.push(`/people/suppliers/list?page=${data.previous}`)
}

function goToNext() {
    if (data.next != null)
        router.push(`/people/suppliers/list?page=${data.next}`)
}
