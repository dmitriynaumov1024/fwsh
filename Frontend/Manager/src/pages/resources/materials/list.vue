    ...
    <Pagination :items="data.items" :page="props.page" 
        ...
        <template v-slot:title>
            ...
            <div class="flex-stripe flex-pad-1 mar-b-1">
                ...
                <router-link to="/resources/materials/create" class="button button-primary">
                    + {{locale.material.single}}
                </router-link>
            </div>
        </template>
        <template v-slot:repeating="{ item }">
            <MaterialView :mat="item" @click="()=> goToItem(data, item)" class="card pad-1 mar-b-1" />
        </template>
    </Pagination>
...
function goToPage (page) {
    ...
    if (page != null) router.push(`/resources/materials/list?${qs.stringify(query)}`)
}
