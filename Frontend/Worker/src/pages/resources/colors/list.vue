...
<Fetch url="/resources/colors/list"
    ...
    <Pagination :items="data.items" :page="props.page" 
        ...
        <template v-slot:title>
            <h2 class="margin-bottom-1">{{locale.color.plural}} &ndash; {{locale.common.page}} {{props.page}}</h2>
        </template>
        <template v-slot:repeating="{ item }">
            <ColorView :color="item" @click="goToItem(item)" class="card-card pad-1 margin-bottom-1" />
        </template>
    </Pagination>
    </template>
</Fetch>
