...
<ProductionTaskView v-if="data.task" 
    :task="data.task"
...
function getTask () {
    axios.get({
        url: `/worker/tasks/production/view/${props.id}`,
        cacheTTL: 1
    })
...
function setStatus (status) {
    ...
    axios.post({
        url: `/worker/tasks/production/set-status/${props.id}`,
...
function submitUsage () {
    axios.post({
        url: `/worker/tasks/production/set-usage/${props.id}`,
